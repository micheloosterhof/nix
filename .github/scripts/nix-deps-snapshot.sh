#!/usr/bin/env bash
# ABOUTME: Emit a GitHub Dependency Submission API snapshot for a Nix system
# ABOUTME: closure. Usage: nix-deps-snapshot.sh <host> <result-symlink> > snapshot.json
#
  ...
  jq --raw-input --slurp \
    ...
    | {
        ...
        manifests: {
          ($host): {
            name: $host,
            file: { source_location: ("modules/hosts/" + $host + ".nix") },
            resolved: $resolved
          }
        }
      }
